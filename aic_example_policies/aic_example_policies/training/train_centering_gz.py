"""Train the Phase 1 (centering) SAC policy using the live Gazebo simulation.

Unlike train_centering.py (MuJoCo), this script requires the eval container to be
...
Note: Gazebo training is ~100× slower than MuJoCo (real-time vs. accelerated sim).
      Reduce --total_steps accordingly or run overnight.
"""
...
from .gazebo_centering_env import GazeboCenteringConfig, GazeboCenteringEnv
...
def parse_args():
    ...
    p.add_argument("--save_dir", default="checkpoints/gazebo/phase1")
    ...
    p.add_argument("--det_loss_weight", type=float, default=0.1,
                   help="Weight for port detection auxiliary loss (0 to disable)")
...
def main():
    ...
    cfg = GazeboCenteringConfig(
        port_type=args.port_type,
        port_frame=args.port_frame,
    )
    env = GazeboCenteringEnv(cfg)
    ...
    print(
        f"Training Phase 1 in Gazebo on {DEVICE}. "
    ...
    while total_steps < args.total_steps:
        ...
        # GT detection label from TF (may be None if TF unavailable)
    ...
    print("Gazebo Phase 1 training complete.")
    env.close()
